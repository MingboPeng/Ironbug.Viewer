...
export class IBLoopShapeUtil extends BaseBoxShapeUtil<IBLoopShape> {
  ...
  override component(shape: IBLoopShape) {
    ...
    const separatorPathData =
      "M" + separator[0] + "L" + separator.slice(1) + "Z";
...
function getHouseVertices(shape: IBLoopShape): {
  separator: Vec[];
  arrowLeft: Vec[];
  arrowRight: Vec[];
} {
  ...
  const halfH = h / 2;
  const separator = [new Vec(0, halfH), new Vec(w, halfH)];
  const arrowLeft = [new Vec(0, h), new Vec(0, 0)];
  const arrowRight = [new Vec(w, 0), new Vec(w, h)];
